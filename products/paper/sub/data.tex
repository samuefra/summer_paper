...
%Seja $H_{f}^{br}$ o vetor de tickers que estão acima da breakpoint rule e $L_{f}^{br}$ o vetor de tickers que estão abaixo da breakpoint rule para algum factor específico, onde o superescrito denota a breakpoint rule $br = m, t$ ($m$ representa mediana como breakpoint rule e $t$ representa os três decis como breakpoint rule) e o subescrito denota o fator $f$. Denote também por $r_i$ o retorno da ação da firma $i$ e $\omega_i \in \omega$ o peso baseado no Market Cap da firma $i$.
